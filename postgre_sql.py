...
def insert_translation(source_text, source_lang, target_text, target_lang, domain=None, note=None):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("""
    INSERT INTO glossary (source_text, source_lang, target_text, target_lang, domain, note)
    VALUES (%s, %s, %s, %s, %s, %s)
    """, (source_text, source_lang, target_text, target_lang, domain, note))
    conn.commit()
    cur.close()
    conn.close()

# Function to get translation if exists
def get_translation(source_text, source_lang='en', target_lang='zh'):
    conn = get_connection()
    cur = conn.cursor()
...
if __name__ == "__main__":
    print(find_terms_in_text("本发明公开了一种马桶防漏水干湿分离机构，包括马桶主体和接水盘，所述马桶主体的上部设有便盆，该便盆的底部具有排泄通道，所述接水盘能够水平方向转动地安装在马桶主体上，从而使接水盘能够沿水平方向在位于排泄通道正下方的第一位置和位于排泄通道以外的第二位置之间来回转动，还包括安装在接水盘和/或排泄通道上的防漏水结构。", "zh-hans"))